...
# 			rms_error10n = rms_error10.cpu().detach().numpy()
	...
	def dataprocessor(self, imu_data, kin_data):
		...
		#trainf_val = [x for x in all_val if x not in test_val]
	...
	def load_mat(self):
		...
		for cur_mat in self.cur_data:
			...
			if not "normalized" in cur_mat:
				continue
			if cur_mat.endswith("05.mat") or cur_mat.endswith("10.mat") or cur_mat.endswith("09.mat") or cur_mat.endswith("11.mat") or cur_mat.endswith("mint.mat"):
				continue
