...
from model_combination import Model
device=torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
if torch.cuda.is_available():
	torch.set_default_tensor_type(torch.cuda.FloatTensor)
class trainer():
	def __init__(self, config, imu_comb):
	...
	def valid(self, train_imu_ten, train_dyn_ten, val_imu_ten, val_dyn_ten):
		...
		weight_name = os.path.join('./weights', self.weights_path + '_' + self.icstr + '.pt')
	...
	def test_lr(self, test_imu_ten, test_dyn_ten, data_labels, model):
		...
		for i_batch in range(y_predictedt.shape[0]):
			...
			if self.save_result:
				...
				create_dir(os.path.join('results', self.result_path, self.icstr))
				fpath1 = os.path.join('results', self.result_path, self.icstr, fname1)
				fpath2 = os.path.join('results', self.result_path, self.icstr, fname2)
				fpath3 = os.path.join('results', self.result_path, self.icstr, fname3)
				fpath4 = os.path.join('results', self.result_path, self.icstr, fname4)
				fpath5 = os.path.join('results', self.result_path, self.icstr, fname5)
				fpath6 = os.path.join('results', self.result_path, self.icstr, fname6)
				fpath7 = os.path.join('results', self.result_path, self.icstr, fname7)
				fpath8 = os.path.join('results', self.result_path, self.icstr, fname8)
				fpath9 = os.path.join('results', self.result_path, self.icstr, fname9)
				fpath10 = os.path.join('results', self.result_path, self.icstr, fname10)
				fpathe = os.path.join('results', self.result_path, self.icstr, fnamee)
	...
	def dataprocessor(self, imu_data, kin_data):
		...
		tmp_test_imu = imu_data[6:7]
		tmp_val_imu = imu_data[5:6]
		tmp_train_dyn = kin_data[0:5]
		tmp_test_dyn = kin_data[6:7]
